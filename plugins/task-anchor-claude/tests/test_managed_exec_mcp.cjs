...
test("initialize, ping, tools/list, and notifications follow JSON-RPC contract", async () => {
  ...
  assert.match(inputSchema.properties.operation.description, /lines/);
  assert.match(inputSchema.properties.operation.description, /cwd/);
  assert.match(inputSchema.properties.cwd.description, /output/);
  assert.match(inputSchema.properties.cwd.description, /不需要 cwd/);
  assert.match(inputSchema.properties.run_id.description, /stop/);
  assert.match(inputSchema.properties.run_id.description, /不需要 cwd/);
  assert.equal(typeof inputSchema.properties.include_keep.description, "string");
  assert.equal(inputSchema.properties.stop_policy.default, "cleanup");
