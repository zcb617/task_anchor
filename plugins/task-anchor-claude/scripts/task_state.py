...
SKILL_MARKER = "/task-anchor:task-anchor"
SKILL_PATTERN = re.compile(r"(?<![A-Za-z0-9_:-])/task-anchor:task-anchor(?![A-Za-z0-9_:-])")
END_SKILL_MARKER = "/task-anchor:task-anchor-end"
END_SKILL_PATTERN = re.compile(
    r"(?<![A-Za-z0-9_:-])/task-anchor:task-anchor-end(?![A-Za-z0-9_:-])"
)
READ_ONLY_SKILL_MARKER = "/task-anchor:task-anchor-readonly"
READ_ONLY_SKILL_PATTERN = re.compile(
    r"(?<![A-Za-z0-9_:-])/task-anchor:task-anchor-readonly(?![A-Za-z0-9_:-])"
)
WRITE_SKILL_MARKER = "/task-anchor:task-anchor-write"
...
def read_post_compact_reload_count() -> int:
    """读取 Claude Code PostCompact 提醒条数配置，异常时返回默认值。"""
    try:
...
def post_compact_continuity_reminder() -> str:
    """按当前配置生成 Claude Code PostCompact 连续性提醒文本。"""
    reload_count = read_post_compact_reload_count()
    return (
        "你刚刚经历了上下文压缩，请立刻重新读取CLAUDE.md规则文件，以及最近的"
...
def set_mutation_policy(
    ...
    *,
    ...
    if data_root is None:
        return warning("Task Anchor 无法切换只读门控：Hook 未提供持久化数据目录。")
    current_session_id = read_session_id(data)
    if current_session_id is None:
...
def start_new_task(
    ...
    *,
    ...
    if not isinstance(instruction, str) or not instruction.strip():
        return warning("Task Anchor 需要非空的任务指令。")

    if data_root is None:
        return warning("Task Anchor 无法创建任务：Hook 未提供持久化数据目录。")
    current_session_id = read_session_id(data)
    if current_session_id is None:
    ...
    try:
        ...
        return warning(
            "Task Anchor 无法确认新任务已建立；请重新显式调用 /task-anchor:task-anchor。"
        )
...
def end_current_task(
    ...
    if data_root is None:
        return warning("Task Anchor 无法结束任务：Hook 未提供持久化数据目录。")
    current_session_id = read_session_id(data)
    if current_session_id is None:
        ...
        with session_lock(session_directory(data_root, current_session_id)):
            if read_pending_transition(data_root, current_session_id) is not None:
                ...
                return warning(
                    ...
                    "请稍后重新调用 /task-anchor:task-anchor-end。"
                )
...
def _is_managed_exec_tool(tool_name: str) -> bool:
    ...
    return normalized in {item.lower() for item in MANAGED_EXEC_TOOL_NAMES}
...
def _bind_managed_exec_to_session(
    ...
    if current_session_id is None:
        ...
        return None
    if tool_input is None or not isinstance(cwd, str) or not cwd.strip():
        write_audit_event(
            data_root,
            data,
            "managed_exec_bind_skipped",
    ...
    return {
        "hookSpecificOutput": {
            ...
            "permissionDecisionReason": "bound managed_exec to the current session and workspace",
...
def guard_pre_tool_use(
    ...
    if _is_managed_exec_tool(tool_name):
        binding = _bind_managed_exec_to_session(data, data_root)
        return binding or _deny_read_only(
            "managed_exec requires a verified hook session and workspace."
        )

    if tool_name.strip().lower().endswith("__managed_exec"):
        return _deny_read_only(
